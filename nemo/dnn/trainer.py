        ...
        for curr_step, imgs in enumerate(train_dataset.take(num_epochs * num_steps_per_epoch)):
            ...
            if (curr_step + 1) % num_steps_per_epoch == 0:
                ...
                with self.writer.as_default():
                    tf.summary.scalar('Loss', loss_value, step=curr_epoch * num_steps_per_epoch)
                    tf.summary.scalar('PSNR_SR', avg_sr_psnr, step=curr_epoch * num_steps_per_epoch)
                    tf.summary.scalar('PSNR_Bilinear', avg_bilinear_psnr, step=curr_epoch * num_steps_per_epoch)
                    tf.summary.scalar('PSNR_Gain', avg_sr_psnr - avg_bilinear_psnr, step=curr_epoch * num_steps_per_epoch)
                    self.writer.flush()
