...
def decode_and_resize_image(image_file, image_shape):
    # 检查文件是否存在
   # if not tf.io.gfile.exists(image_file):
...
#    if img.shape.ndims < 2:
 #       raise ValueError("Decoded image has no shape: {}".format(img.shape))

    # 检查解码后的图像是否有效
   # if img is None or img.shape.ndims is None:
      #  raise ValueError(f"Decoded image is invalid: {img}")
    # 获取图像的动态形状
   # img_shape = tf.shape(img)

    # 检查解码后的图像是否有效
   # if img_shape[0] == 0 or img_shape[1] == 0:
    #    raise ValueError("Decoded image is invalid: shape is zero")


    # 打印解码后的图像形状（用于调试）
   # print(f"Decoded image shape: {img.shape}")

    # 确保图像是四维张量，形状为 [batch_size, height, width, channels]
   # if img.shape.ndims == 2:  # 如果是2D图像（高度和宽度），添加颜色通道维度
   #     img = tf.expand_dims(img, axis=-1)
    img = tf.image.decode_image(image_file, channels=image_shape[2],expand_animations=False)
    # 检查图像是否为空或形状不正确
#    if img is None or tf.shape(img) == [0]:
 #       print(f"警告: 图像为空或无法解码。返回零张量。")
  #    return tf.zeros([image_shape[0], image_shape[1], image_shape[2]], dtype=tf.uint8)  # 返回指定形状的零张量
     # 使用 tf.cond 来检查图像是否有效
    def return_zero_tensor():
        print(f"警告: 图像为空或无法解码，返回零张量。")
        return tf.zeros([image_shape[0], image_shape[1], image_shape[2]], dtype=tf.uint8)
  ...
  # img = tf.expand_dims(img, 0)
   # img = tf.image.resize_bilinear(img, (image_shape[0], image_shape[1]), half_pixel_centers=True)
    img = tf.image.resize(img,(image_shape[0], image_shape[1]), method='bilinear')
...
#    img = tf.image.decode_image(image_file, channels=image_shape[2])
    ...
    # 确保图像是二维（高度和宽度）或三维（带颜色通道）
 #   if img.shape.ndims == 2:
  #      img = tf.expand_dims(img, axis=-1)  # 添加颜色通道维度

    # 调整图像形状
   # img = tf.image.resize(img, size=(image_shape[0], image_shape[1]), method='bilinear')
