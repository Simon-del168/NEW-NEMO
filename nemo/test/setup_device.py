...
if __name__ == '__main__':
    ...
    cmds = ['#!/system/bin/sh',
            'export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:{}'.format(device_lib_dir),
            'cd {}'.format(device_root_dir),
            '{} --codec=vp9  --noblit --threads={} --frame-buffers=50 {} --dataset-dir={} --input-video-name={}  --decode-mode=decode_sr --dnn-mode=online_dnn --dnn-runtime=gpu_float32 --dnn-name={} --dnn-scale={} --save-latency --save-metadata'.format(os.path.join(device_bin_dir, 'vpxdec_nemo_ver2'), get_num_threads(input_height), limit, device_root_dir, args.video_name, model.name, scale),
            'exit']
    ...
    cmds = ['#!/system/bin/sh',
            'export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:{}'.format(device_lib_dir),
            'cd {}'.format(device_root_dir),
            '{} --codec=vp9  --noblit --threads={} --frame-buffers=50 --dataset-dir={} --input-video-name={} --decode-mode=decode_cache --dnn-mode=online_dnn --dnn-runtime=gpu_float32 --cache-mode=profile_cache --dnn-name={} --dnn-scale={} --cache-profile-name={} --save-latency --save-metadata'.format(os.path.join(device_bin_dir, 'vpxdec_nemo_ver2'), get_num_threads(input_height), device_root_dir, args.video_name, model.name, scale, args.algorithm),
            'exit']
