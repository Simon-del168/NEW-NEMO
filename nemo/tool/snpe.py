...
# Disable eager execution for compatibility with the existing code
...
def snpe_benchmark(json_file):
    check_python_version()
    setup_cmd = 'source {}/bin/envsetup.sh -t {}'.format(SNPE_ROOT, TENSORFLOW_ROOT)
    snpe_cmd = 'python {}/benchmarks/SNPE/snpe_bench.py -c {} -json'.format(SNPE_ROOT, json_file)
...
def snpe_convert_model(model, input_shape, checkpoint_dir):
    ...
    tf.compat.v1.train.write_graph(frozen_graph, checkpoint_dir, pb_name, as_text=False)
