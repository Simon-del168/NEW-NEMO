...
class ONNXModelUtils(FrameworkModelAPI):
    ...
    def optimize_by_ort(
        ...
        fork_status, model_wrapper = fork_process_and_exec(
    ...
    def update_output_names(self, output_names: List[Text]):
        ...
        while queue:
            ...
            if current_node.name not in nodes_to_delete:
                ...
                log_debug("{} node is not present. Might have already removed from map".format(current_node.name))
                continue
        ...
        log_debug("Total nodes remained in nodes_to_delete {}".format(len(nodes_to_delete)))
    ...
    def add_inputs(self, input_names: List[Text], infer_shape: bool = True):
        ...
        val_info_dict = Utils.get_value_info_proto_mapping(self.loader._model)
        init_dict = Utils.get_initializer_mappings(self.loader._model)
    ...
    def add_outputs(self, output_names: List[Text], infer_shape: bool = True):
        ...
        tensor_val_info_dict = Utils.get_value_info_proto_mapping(self.loader._model)
    ...
    def _native_shape_inference_helper(self, delete_existing_shapes) -> ModelWrapper:
        ...
        try:
            ...
            # Remove temp onnx file and external data file incase created during Symbolic shape inference Failure
            if os.path.exists("sym_shape_infer_temp.onnx"):
                os.remove("sym_shape_infer_temp.onnx")
                current_dir = os.getcwd()
                files = os.listdir(current_dir)
                current_dir_files_paths = [os.path.join(current_dir, basename) for basename in files]
                # Fetch the external data file based on latest created file using timestamp
                ext_data_file_abspath = max(current_dir_files_paths, key=os.path.getctime)
                ext_data_file_path = os.path.basename(ext_data_file_abspath)
                # Double check to not remove the wrong file
                if (os.path.exists(ext_data_file_path) and '.' not in ext_data_file_path and ext_data_file_path.count('-') == 4):
                    os.remove(ext_data_file_path)
