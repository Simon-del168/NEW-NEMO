...
### [QTI Change]
from .common import ExprTable, logger
from .common import get_enclosed_nodes_as_list
### [End QTI Change]
...
class OperatorConverter(object):
    ...
    ### [QTI Change]
    def __init__(self, model, subgraph, exp_tab, shape_dict, output_node_names):
    ### [End QTI Change]
        ...
        self.updated_op_indices_list = None
        ### [End QTI Change]
        ...
        self.convert_map = {
            ...
            ### [QTI Change]
            "FlexFakeQuantWithMinMaxVarsPerChannel": self.convert_flex_fake_quant,
            "FlexMul": self.convert_mul,
            ### [End QTI Change]
    ...
    ### [QTI Change]
    def check_unsupported_ops(self, custom_op_factory=None):
    ### [End QTI Change]
        ...
        if custom_op_factory and len(custom_op_factory.op_collection):
            known_ops += [key for key in custom_op_factory.op_collection.__dict__.keys() if key != 'op_count']
        op_indices_list = range(self.subgraph.OperatorsLength()) if not self.updated_op_indices_list else \
                          self.updated_op_indices_list
        for op_idx in op_indices_list:
            op = self.subgraph.Operators(op_idx)
            op_code_str = self.get_op_code_str(op)
            if op_code_str not in known_ops:
    ...
    ### [QTI Change]
    def update_converted_node_list(self):
        graph_input_names = [str(get_tensor_index(self.subgraph, tensor_name)) for tensor_name in self.shape_dict.keys()]
        ordered_node_dict = OrderedDict()

        for op_idx in range(self.subgraph.OperatorsLength()):
            op = self.subgraph.Operators(op_idx)
            input_tensor_names = list()
            for tensor_wrapper in self.get_input_tensors(op):
                from tflite.Buffer import Buffer
                if isinstance(tensor_wrapper.buffer, Buffer) and tensor_wrapper.buffer.DataLength() == 0:
                    # Tensor without static data in the graph
                    input_tensor_names.append(str(tensor_wrapper.tensor_idx))
                else:
                    pass

            output_tensor_names = list()
            for tensor_wrapper in self.get_output_tensors(op):
                output_tensor_names.append(str(tensor_wrapper.tensor_idx))

            ordered_node_dict[str(op_idx)] = {'inputs': input_tensor_names,
                                              'outputs': output_tensor_names}

        self.updated_op_indices_list = get_enclosed_nodes_as_list(ordered_node_dict,
                                                                  graph_input_names,
    ...
    ### [QTI Change]
    def convert_op_to_relay(self, custom_op_factory=None):
    ### [End QTI Change]
        """
        ...
        ### [End QTI Change]
            ...
            ### [QTI Change]
            output_names = [get_tensor_name(self.subgraph, tensor.tensor_idx) for tensor in output_tensors]
            if op_code_str == "CUSTOM":
                op_code_str = self.get_op_code_for_custom(output_names, custom_op_factory)
            if custom_op_factory and op_code_str in custom_op_factory.op_collection:
                if op_code_str in custom_ops_count_dict:
                    custom_ops_count_dict[op_code_str] += 1
                else:
                    custom_ops_count_dict[op_code_str] = 0
                node_name = op_code_str + "_" + str(custom_ops_count_dict[op_code_str])
                # custom_op_dict is a dict that contains the node-name as the key and the TfliteCustomOp object as the value.
                # It is referenced in convert_custom as the custom_tflite_op
                custom_ops_dict[node_name] = custom_op_factory.op_collection[op_code_str][custom_ops_count_dict[op_code_str]]
                ret = self.convert_map['CUSTOM'](op, node_name, custom_ops_dict)
            elif op_code_str in self.convert_map.keys():
            ...
            ### [QTI Change]
            op_name = '' # there is no op_name in tflite frontend
            output_qparams = [self.get_tensor_qparams(tensor.tensor_idx) for tensor in output_tensors]
            ret = set_span(ret, op_name, op_type=op_code_str, output_names=output_names, output_qparams=output_qparams)

            if len(output_tensors) == 1:
                self.exp_tab.set_expr(output_names[0], ret)
            else:
                for idx, output_tensor in enumerate(output_tensors):
                    self.exp_tab.set_expr(
                        output_names[idx], ret[idx]
                    )
            ### [End QTI Change]
    ...
    ### [QTI Change]
    def get_op_code_for_custom(self, output_names, custom_op_factory=None):
        if custom_op_factory:
            op_defs = custom_op_factory.custom_opdefs
            for i in range(len(output_names)):
                for j in range(len(op_defs)):
                    op_def_outputs = op_defs[j].output
                    for k in range(len(op_def_outputs)):
                        if len(op_def_outputs[k].name) > 0 and (output_names[i] == op_def_outputs[k].name
                                or output_names[i].rsplit('_', 1)[0] == op_def_outputs[k].name):
                            corresponding_op_name = op_defs[j].type_name
                            return corresponding_op_name
    ### [End QTI Change]
            ...
            if tflite_qnn_params is not None:
                ...
                if isinstance(tflite_scale, np.ndarray):
                    ...
                    if tflite_scale.size != 1 and tflite_zero_point.size != 1:
                        scale = tflite_scale
                        ### [QTI Change]
                        # Ensure that all zero points are the same
    ...
    def convert_reshape(self, op):
        ...
        if len(input_tensors) == 2:
            ...
            if self.has_expr(shape_tensor.tensor_idx):
                ### [QTI Change]
                tensor_name = shape_tensor.tensor.Name().decode('utf-8')
                if tensor_name in self.exp_tab.params:
                    target_shape = list(self.get_tensor_value(shape_tensor))
                else:
                ### [End QTI Change]
    ...
    def convert_gather(self, op):
        ...
        if self.has_expr(indices.tensor_idx):
            ...
            indices_expr = self.exp_tab.new_const(
                ...
                output_names=[indices.tensor.Name()],
                output_qparams=[self.get_tensor_qparams(indices.tensor_idx)]
            )
            ### [End QTI Change]
        ...
        def _transform_mask(stride_dim, ellipsis_mask):
            ...
            for index in range(stride_dim):
                ...
                if mask & ellipsis_mask:
                    ...
                    ### [QTI Change]
                    # the original calculation of start and end is wrong.
                    # e.g., given data = [0,1,2], if we give mask=begin_mask=end_mask=1 and stride=-1, the start and end are calculated as 3 and 0
                    # and if we use that to slice a data, data[3:0:-1] return [2,1].
                    # Instead, we should caluclated start as 2 and end as -4, where -4 comes from "-1 - dimension" to avoid canonicalization
                    # e.g., data[2:-1:-1] is canonicalized into data[2:2:-1] which return [],
                    # so we use -1-dimension = -4 and data[2:-4:-1] is canonicalized into data[2:-1:-1] which return [2,1,0]
                    if final_index == len(m_begin):
                        break
                    if mask & begin_mask:
                        m_begin[final_index] = data_shape[final_index]-1 if stride[index] < 0 else 0
                    elif begin[index]:
                        m_begin[final_index] = begin[index]
                    if mask & end_mask:
                        m_end[final_index] = -(data_shape[final_index]+1) if stride[index] < 0 else data_shape[final_index]
                    elif end[index]:
    ...
    def convert_fully_connected(self, op):
        ...
        if self.has_expr(weight_tensor.tensor_idx):
            ...
            weight_expr = self.exp_tab.new_const(
                ...
                output_names=[weight_tensor.tensor.Name()],
                output_qparams=[self.get_tensor_qparams(weight_tensor.tensor_idx)]
            )
            ### [End QTI Change]
            ...
            if bias_tensor.tensor_idx != -1:
                ...
                if self.has_expr(bias_tensor.tensor_idx):
                    ...
                    bias_expr = self.exp_tab.new_const(
                        ...
                        output_names=[bias_tensor.tensor.Name()],
                        output_qparams=[self.get_tensor_qparams(bias_tensor.tensor_idx)]
                    )
                    ### [End QTI Change]
        ...
        if output_tensor.qnn_params:
            ...
            if isinstance(weight_scale, float):
                # Weight scale is a scalar, it is a case of per-tensor quantization
                weight_scale_val = get_scalar_from_constant(weight_scale)
            else:
                # Weight Scale is a tensor, it is a case of per-channel quantization
        ...
        if keep_num_dims and len(input_tensor_shape) != 2:
            ### [QTI Change]
            input_shape = tuple(input_tensor_shape)
            ### [End QTI Change]
    ...
    def convert_scatter_nd(self, op):
        ...
        shape = self.get_tensor_value(input_tensors[2])

        data_dtype = _infer_type(updates).checked_type.dtype
        data = _op.const(np.zeros(shape, dtype=data_dtype), dtype=data_dtype,)

        out = _op.scatter_nd(data, indices, updates)
    ...
    def convert_pad(self, op):
        ...
        ### [QTI Change]
        # add default layout for TFLite pad op
        data_layout_dict = {
            2: "NF",
            3: "NWC",
            4: "NHWC",
        ...
        dim = len(self.get_tensor_shape(input_tensor))
        data_layout = data_layout_dict[dim]

        out = _op.nn.pad(in_expr, pad_width=paddings, pad_value=pad_value, data_layout=data_layout)
        ### [End QTI Change]
        return out
    ...
    def convert_batch_matmul(self, op):
        ...
        ### [QTI Change]
        if rank_b == 2:
            return _op.nn.matmul(input_a,
                                 input_b,
                                 transpose_a=batch_matmul_options.AdjX(),
                                 transpose_b=batch_matmul_options.AdjY())
        else:
            ...
            input_a = self.flatten_to_nd(input_a, shape_of(input_a), 3)
            input_b = self.flatten_to_nd(input_b, shape_of(input_b), 3)
            ### [End QTI Change]
    ...
    def convert_broadcast_to(self, op):
        """broadcast_to implementation."""

        input_tensors = self.get_input_tensors(op)
        assert len(input_tensors) == 2, "two input tensor arguments expected"

        in_expr = self.get_expr(input_tensors[0].tensor_idx)
        broadcast_shape = list(self.get_tensor_value(input_tensors[1]))

        out = _op.broadcast_to(in_expr, broadcast_shape)
    ...
    def convert_prelu(self, op):
        ...
        if self.has_expr(alpha_tensor.tensor_idx):
            ...
            alpha_expr = self.exp_tab.new_const(
                self.get_tensor_value(alpha_tensor).flatten(),
                dtype=alpha_tensor_type_str,
                output_names=[alpha_tensor.tensor.Name()],
                output_qparams=[self.get_tensor_qparams(alpha_tensor.tensor_idx)]
            )
        in_expr = self.get_expr(input_tensor.tensor_idx)
        data_shape = to_int_list(self.get_tensor_shape(input_tensor))
        ### [QTI Change]
        # tensorflow prelu support multiple dimension alpha
        # check https://www.tensorflow.org/api_docs/python/tf/keras/layers/PReLU
        # e.g.,
        # in_expr: [1, 96, 96, 32]
        # alpha_expr: [1, 1, 32]
        alpha_shape = _infer_shape(alpha_expr)
        alpha_expr = _op.expand_dims(alpha_expr, axis=0, num_newaxis=len(data_shape)-len(alpha_shape))
        out = _op.nn.prelu(in_expr, alpha_expr, axis=len(data_shape)-1)
        ### [End QTI Change]
        return out
    ...
    def convert_transpose_conv(self, op):
        ...
        if self.has_expr(weights_tensor.tensor_idx):
            ...
            weight_expr_iohw = self.exp_tab.new_const(
                weight_value_iohw,
                dtype=weight_tensor_type_str,
                output_names=[weights_tensor.tensor.Name()],
                output_qparams=[self.get_tensor_qparams(weights_tensor.tensor_idx)]
            )
            ### [End QTI Change]
    ...
    def convert_dequantize(self, op):
        ...
        if input_tensor.tensor.Type() == TensorType.FLOAT16:
            ...
            if self.has_expr(input_tensor.tensor_idx):
                ...
                if self.is_prefetched(input_tensor.tensor_idx):
                    input_value = self.get_prefetched_node(input_tensor.tensor_idx)
                else:
                ...
                in_expr = self.exp_tab.new_const(
                    input_value,
                    dtype=dtype,
                    output_names=[input_tensor.tensor.Name()],
                    output_qparams=[self.get_tensor_qparams(input_tensor.tensor_idx)]
                )
            ### [End QTI Change]
    ...
    def convert_detection_postprocess(self, op):
        ...
        anchor_expr = self.exp_tab.new_const(
            anchor_values,
            dtype=anchor_type,
            output_names=[inputs[2].tensor.Name()],
            output_qparams=[self.get_tensor_qparams(inputs[2].tensor_idx)]
        )
        ### [End QTI Change]
        ...
        # follow https://github.com/tensorflow/tensorflow/blob/master/tensorflow/lite/kernels/detection_postprocess.cc#L644
        # we calculate label offset to get "background_class_idx" and pass it to tvm pass,
        # since we can't pass the value to tvm pass directly, and tflite don't use attribute "id_index" in "non_max_suppression" currently,
        # we store "background_class_idx" in "id_index" as a workaround to pass it to tvm pass
        # [End QTI Change]
        non_max_suppression_attrs["id_index"] = inputs[1].tensor.ShapeAsNumpy()[2] - custom_options["num_classes"] - 1
    ...
    ### [QTI Change]
    def convert_flex_fake_quant(self, op):
        """Convert TFLite FLEX_FAKE_QUANT"""
        input_tensors = self.get_input_tensors(op)
        assert len(input_tensors) == 3, "input tensors length should be 3"

        input_tensor = input_tensors[0]
        input_expr = self.get_tensor_expr(input_tensor)
        return input_expr
    ### [End QTI Change]

    ### [QTI Change]
    def convert_custom(self, op, node_name, custom_ops_dict):
        from qti.aisw.converters.backend.custom_ops.core import qnn_to_native_dtype
        from qti.aisw.converters.backend.custom_ops.core import TensorInfo

        def custom_op_rel(arg_types, attrs):
            if not custom_ops_dict:
                logger.warning("Custom op is not parsed, copy output rel from input")
                return tvm.relay.TupleType(arg_types)

            custom_tflite_op = custom_ops_dict[str(attrs['node_source_name'])]

            output_dims = custom_tflite_op.output_dims
            snpe_to_dtype = TensorInfo()
            snpe_dtype = snpe_to_dtype.snpe_qnn_udo_op_dict
            output_type = [tvm.relay.TensorType(output_dim, qnn_to_native_dtype(snpe_dtype[output.data_type]))
                           for output_dim, output in zip(output_dims, custom_tflite_op.outputs)]

            return tvm.relay.TupleType(output_type)


        op_code_str = node_name.rsplit('_',1)[0]
        input_tensors = self.get_input_tensors(op)
        assert len(input_tensors) > 0, "input tensors should be 1 or greater"
        inputs = []
        for i in range(len(input_tensors)):
            ...
            if self.has_expr(input_tensor.tensor_idx):
                try:
                    ...
                    inputs.append(self.get_tensor_expr(input_tensor.tensor_idx))
            else:
                if self.is_prefetched(input_tensor.tensor_idx):
                    inputs.append(self.get_prefetched_node(input_tensor.tensor_idx))
                else:
                    inputs.append(self.get_tensor_value(input_tensor))
        output_tensors = self.get_output_tensors(op)
        assert len(output_tensors) > 0, "output tensors should be 1 or greater"
        output_names = []
        for i in range(len(output_tensors)):
            output_tensor = output_tensors[i]
            output_names.append(output_tensor.tensor.Name())

        try:
            _op.op.register(op_code_str)
            _op.op.register_pattern(op_code_str, _op.OpPattern.OPAQUE)
            _op.op.register_stateful(op_code_str, False)
        except tvm.error.TVMError as e:
            pass
        new_inputs = [inp for inp in inputs if isinstance(inp, _expr.Expr)]
        custom_op = _op.op.get(op_code_str)
        custom_op.set_num_inputs(len(new_inputs))
        custom_op.set_attrs_type_key("DictAttrs")
        custom_op.add_type_rel(op_code_str, custom_op_rel)
        custom_op.set_support_level(1)
        new_attrs = {'node_source_name': node_name}
        call_attrs = tvm.ir.make_node("DictAttrs", **new_attrs)
        custom_call_node = tvm.relay.Call(
            custom_op,
            new_inputs,
            call_attrs
        )
        return tvm.relay.Tuple(
        tuple(
            tvm.relay.TupleGetItem(custom_call_node, idx)
    ...
    def get_tensor_expr(self, tensor, is_sparse=False):
        ...
        if self.has_expr(tensor.tensor_idx):
            ...
            expr = self.exp_tab.new_const(
                ...
                dtype=type_str,
                output_names=[tensor.tensor.Name()],
                output_qparams=[self.get_tensor_qparams(tensor.tensor_idx)]
            )
            ### [End QTI Change]
        return expr
    ...
    ### [QTI Change]
    def get_tensor_qparams(self, input_tensor_idx):
        return get_tensor_qparams_as_dict(self.subgraph, input_tensor_idx)
    ### [End QTI Change]
...
### [QTI Change]
def get_tensor_index(subgraph, tensor_name):
    ...
    for tensor_idx in range(subgraph.TensorsLength()):
        if tensor_name == get_tensor_name(subgraph, tensor_idx):
            return tensor_idx
### [End QTI Change]
...
### [QTI Change]
def get_tensor_qparams_as_dict(subgraph, tensor_idx):
    """Get tensor quantization parameters given TFLite subgraph and tensor index"""
    ...
    if tflite_qnn_params is not None:
        tflite_scale = tflite_qnn_params.ScaleAsNumpy()
        tflite_zero_point = tflite_qnn_params.ZeroPointAsNumpy()
        if isinstance(tflite_scale, np.ndarray) and isinstance(tflite_zero_point, np.ndarray):
            qnn_params = dict()
            qnn_params["scale"] = tvm.nd.array(tflite_scale.astype(np.float32))
            qnn_params["zero_point"] = tvm.nd.array(tflite_zero_point.astype(np.int32))
            qnn_params["dtype"] = _decode_type(tensor.Type())
...
### [QTI Change]
def from_tflite(model, subgraph_name="", shape_dict=None, dtype_dict=None, output_node_names=None,
                op_converter=OperatorConverter, custom_op_factory=None):
### [End QTI Change]
    ...
    output_node_names : list of str
        Output node names of the model.
        The node name refers to node location in tflite model. It is applicable
        to specify node at any location within the model. The output node stands
        for the end point of converted relay graph.
    ...
    # Input shape/dtype inferred from original model
    _shape_dict, _dtype_dict = _input_type(model)
    ### [QTI Change]
    _input_is_modified = False
    if shape_dict is not None:
        if all([input_name in _shape_dict for input_name in shape_dict.keys()]):
            # Replace original input shape with user-specified input shape
            _shape_dict.update(shape_dict)
        else:
            # Shape of intermediate layer specified from user
            _shape_dict = shape_dict
            _input_is_modified = True
    if dtype_dict is not None:
        if all([input_name in _dtype_dict for input_name in dtype_dict.keys()]):
            _dtype_dict.update(dtype_dict)
        else:
            _dtype_dict = dtype_dict
            _input_is_modified = True
    ### [End QTI Change]

    # keep the same as tflite
    subgraph_index = 0
    for i in range(1, model.SubgraphsLength()):
        if model.Subgraphs(i).Name().decode() == subgraph_name:
            subgraph_index = i
            break
    ...
    exp_tab = ExprTable()
    ### [QTI Change]
    model_inputs = list(_shape_dict.keys())
    for model_input_name in model_inputs:
        tensor_idx = get_tensor_index(subgraph, model_input_name)
        tensor = subgraph.Tensors(tensor_idx)
        shape = _shape_dict[model_input_name]
        dtype = _dtype_dict[model_input_name] if model_input_name in _dtype_dict else None
        if not shape:
            shape = to_int_list(tensor.ShapeAsNumpy()) if tensor.ShapeLength() > 0 else None
        if not dtype:
            dtype = _decode_type(tensor.Type())

        model_input_qparams = get_tensor_qparams_as_dict(subgraph, tensor_idx)
        exp_tab.set_expr(model_input_name, set_span(_expr.var(model_input_name, shape=shape, dtype=dtype), output_names=[model_input_name],
                                                    output_qparams=[model_input_qparams]))
    ### [End QTI Change]

    ### [QTI Change]
    # op code in model
    op_converter = op_converter(model, subgraph, exp_tab, _shape_dict, output_node_names)
    if _input_is_modified or output_node_names:
        op_converter.update_converted_node_list()
    op_converter.check_unsupported_ops(custom_op_factory)
    op_converter.convert_op_to_relay(custom_op_factory)
    ### [End QTI Change]
    ...
    if output_node_names:
        # Reset model outputs if customized output names are given
        model_outputs = list()
        for op_idx in output_node_names:
            op = subgraph.Operators(op_idx)
            for tensor_wrapper in op_converter.get_output_tensors(op):
                model_outputs.append(tensor_wrapper.tensor_idx)
    ### [End QTI Change]
