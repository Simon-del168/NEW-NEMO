...
class CumulativeLayerwiseSnooping(Snooper):
    """Class that runs cumulative layerwise snooping."""

    def __init__(self, args, logger, verbose="info"):
        super(CumulativeLayerwiseSnooping, self).__init__(snooping_type="cumulative_layerwise", args=args, logger=logger, verbose=verbose)
        self.step_size = args.step_size

    def add_output_node_to_current_layer(self, model, cur_layer_out_name,
                                         out_count):
        """
    ...
    def run(self, model=None, profile_info=None):
        ...
        qnn_nodes = list(read_json(tensor_mapping).values())
        valid_nodes = [x for x in qnn_nodes if x !='_']
        temp_model = model
        self.logger.info('Started cumulative layerwise snooping')

        valid_node_count = len(valid_nodes)
        node_count_in_final_sub_graph = valid_node_count % self.step_size
        if node_count_in_final_sub_graph > 0:
            num_of_subgraphs = (valid_node_count // self.step_size) + 1
        else:
            num_of_subgraphs = (valid_node_count // self.step_size)
        ...
        while (True):
            ...
            step_count+=1
            if step_count == self.step_size or sub_graph_count == num_of_subgraphs-1:
                step_count = 0
                sub_graph_count+=1
            else:
                continue
            ...
            if not skip_compare:
                ...
                if qnn_model_tensors_json is not None and s_cur_layer_out_name in qnn_model_tensors_json:
                    tensor_dict = qnn_model_tensors_json[s_cur_layer_out_name]
                ...
                if (inf_raw is not None) and (rt_raw is not None):
                    ...
                    for idx, comp in enumerate(comp_list):
                        ...
                        percent_match[comp_name] = round(percent, 4) if not isinstance(percent,str) else 0.0
                        if match_info:
                ...
                for elem in original_outputs:
                    ...
                    qnn_model_json_path = os.path.join(self.output_dir, 'inference_engine',
                                                       s_cur_layer_out_name,
                                                       'model_net.json')
                    if original_output_name not in self.is_transpose_needed_dict:
                        qnn_model_tensors_json = None
                        if qnn_model_json_path is not None:
                            qnn_model_json = read_json(qnn_model_json_path)
                            qnn_model_tensors_json = qnn_model_json['graph']['tensors']

                        if qnn_model_tensors_json is not None and s_original_output_name in qnn_model_tensors_json:
                            tensor_dict = qnn_model_tensors_json[s_original_output_name]
                            src_axis_format = tensor_dict['src_axis_format']
                            axis_format = tensor_dict['axis_format']
                            tensor_dims = tensor_dict['dims']
                    ...
                    for idx, comp in enumerate(comp_list):
                        ...
                        # store percentage match for each user supplied comparator
                        comp_name = comp.V_NAME
                        percent_match_origin[comp_name] = round(percent_origin, 4) if not isinstance(percent_origin,str) else 0.0
                        # maintain a list of over all comparators used in snooping
                        if comp_name not in overall_comp_list:
                            overall_comp_list.append(comp_name)
