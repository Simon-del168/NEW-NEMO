        ...
        if self.transform_pattern == self.Pattern.MATMUL:
            ...
            if converter_context.weights.has(input_names[0]):
                ...
                if not graph.has_buffer(static_input_name):
                    ...
                    self.insert_constant_trace_info(static_input_name, static_input_node, graph)
            ...
            if converter_context.weights.has(input_names[1]):
                ...
                if not graph.has_buffer(weight_input_name):
                    ...
                    self.insert_constant_trace_info(weight_input_name, weight_node, graph)
...
class OnnxGemmTranslation(OnnxTranslationBase):
    ...
    def add_op(self, src_op, converter_context):
        ...
        if graph.has_buffer(self.bias_name) and graph.get_buffer(self.bias_name).rank() == 2:
            ...
            fc_op = op_adapter.FullyConnectedOp(str(src_op.name))
            fc_node = graph.add(fc_op, [self.input_name, self.weights_name], [fc_output_name])
            self.insert_trace_info([fc_node, graph.get_buffer(fc_output_name)], (src_op.name, TraceType.OP), graph)
            ...
            self.insert_trace_info([eleadd_node, graph.get_buffer(output_names[0])], (src_op.name, TraceType.OP), graph)
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if not graph.has_buffer(input_names[1]):
            ...
            if converter_context.weights.has(input_names[1]):
                ...
                self.insert_constant_trace_info(input_names[1], weight_node, graph)
        ...
        if self.params.transB:
            ...
            if not graph.has_buffer(trans_weight_name):
                ...
                self.insert_trace_info([permute_node, graph.get_buffer(trans_weight_name)], (src_op.name, TraceType.OP), graph)
        ...
        if self.params.transA:
            ...
            if not graph.has_buffer(trans_input_name):
                ...
                self.insert_trace_info([permute_node, graph.get_buffer(trans_input_name)], (src_op.name, TraceType.OP), graph)
        ...
        if self.params.alpha != 1:
            ...
            if not graph.has_buffer(alpha_constant_name):
                ...
                self.insert_trace_info([alpha_node, graph.get_buffer(alpha_constant_name)], (src_op.name, TraceType.OP), graph)
            ...
            if not graph.has_buffer(alpha_mul_name):
                ...
                self.insert_trace_info([alpha_mul_node, graph.get_buffer(alpha_mul_name)], (src_op.name, TraceType.OP), graph)
        ...
        if len(src_op.input) >= 3:
            ...
            if self.params.beta != 1:
                ...
                if not graph.has_buffer(beta_constant_name):
                    ...
                    self.insert_trace_info([beta_node, graph.get_buffer(beta_constant_name)], (src_op.name, TraceType.OP), graph)
                ...
                if not graph.has_buffer(beta_mul_name):
                    ...
                    self.insert_trace_info([beta_mul_node, graph.get_buffer(beta_mul_name)], (src_op.name, TraceType.OP), graph)
            ...
            if not graph.has_buffer(self.bias_name):
                ...
                self.insert_trace_info([bias_node, graph.get_buffer(self.bias_name)], (src_op.name, TraceType.OP), graph)
...
class OnnxMatMulTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if len(first_input.shape) == 1:
            ...
            self.insert_trace_info([first_unsqueeze_node, graph.get_buffer(first_unsqueeze_output_name)], (self.input_names[0], TraceType.TENSOR), graph)
        ...
        if len(second_input.shape) == 1:
            ...
            self.insert_trace_info([second_unsqueeze_node, graph.get_buffer(second_unsqueeze_output_name)], (self.input_names[1], TraceType.TENSOR), graph)
...
class OnnxReciprocalTranslation(ElementwiseBinaryTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if len(const_input_data) == len(self.input_names):
            ...
            self.insert_trace_info([const_node, graph.get_buffer(const_op_name)], (src_op.name, TraceType.OP), graph)
...
class OnnxSeluTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if not graph.has_buffer(gamme_name):
            ...
            self.insert_constant_trace_info(gamme_name, gamma_node, graph)
...
class OnnxSoftmaxTranslation(OnnxTranslationBase):
    ...
    def add_op(self, src_op, converter_context, **kwargs):
        ...
        if len(translated_ops) == 3:
            ...
            self.insert_trace_info([pre_reshape_node, graph.get_buffer(pre_reshape_output_names[0])], (src_op.name, TraceType.OP), graph)
            ...
            self.insert_trace_info([softmax_node, graph.get_buffer(softmax_output_names[0])], (src_op.name, TraceType.OP), graph)
            ...
            self.insert_trace_info(post_reshape_node, (src_op.name, TraceType.OP), graph)
            for output_name in output_names:
                self.insert_trace_info(graph.get_buffer(output_name), (output_name, TraceType.TENSOR), graph)
            ...
            self.insert_default_trace_info(src_op, softmax_node, graph)
