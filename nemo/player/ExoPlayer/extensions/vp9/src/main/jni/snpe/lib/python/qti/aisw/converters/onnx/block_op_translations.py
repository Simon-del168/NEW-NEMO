...
class OnnxBlockOpTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context, **kwargs):
        ...
        if src_op.op_type in neuron_types and \
            ...
            return BlockOpAdapterMap.translations[src_op.op_type](str(src_op.name), **param_dict)
        else:
...
OnnxTranslations.register_translation(OnnxBlockOpTranslation(),
                                      converter_type('block_op_MaskedSoftmax', 'onnx'))
OnnxTranslations.register_translation(OnnxBlockOpTranslation(),
                                      converter_type('block_op_Buffer', 'onnx'))
...
class OnnxBlockOpStatefulLstmTranslation(OnnxLSTMTranslation):
    def __init__(self):
        from qti.aisw.converters.common.converter_ir.op_properties.stateful_lstm import (
            NUM_INPUTS, NUM_OUTPUTS, IR_RESET_IDX, IR_NUM_INPUTS, IR_TO_ONNX_INDICES,
        )
        ...
        schema.version = ['1']
        schema._attributes = {'activations': ['activations', 'ls',
                                              ['Sigmoid', 'Tanh', 'Tanh']],
...
OnnxTranslations.register_translation(OnnxBlockOpStatefulLstmTranslation(),
    ...
    def extract_input_names(self, src_op, converter_context):
        ...
        if (len(self.input_names) > self.ONNX_RESET_IDX
            ...
            if len(base_gru_names) == self.IR_NUM_INPUTS:
                base_gru_names[self.IR_RESET_IDX] = self.input_names[self.ONNX_RESET_IDX]
...
OnnxTranslations.register_translation(OnnxBlockOpStatefulGruTranslation(),
                                      converter_type('block_op_StatefulGru', 'onnx'))
