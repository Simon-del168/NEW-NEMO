...
from qti.aisw.accuracy_debugger.lib.utils.nd_errors import get_message, get_warning_message, get_debugging_message, framework_model_exceeds_2GB_error
...
class OnnxFramework_1_3_0(BaseFramework):
    ...
    def check_model_size(self, model_path):
        ...
        try:
            ...
            if any(s in str(e) for s in framework_model_exceeds_2GB_error):
                self.logger.info(str(e))
                self.is_model_greater_than_two_gb = True
                self.logger.warning("Model is greater than 2GB in size. Proceeding accordingly.")
    ...
    def add_outputs(self, output_list=[]):
    ...
    def run_inference(self, input_data, input_tensor_names, output_tensor_names):
        ...
        if self.is_model_greater_than_two_gb:
            ort_session = onnxruntime.InferenceSession(self.model_path, ort_sess_opts)
        else:
